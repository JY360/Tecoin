...
const QString BITCOIN_IPC_PREFIX("tecoin:");
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click heavycoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start heavycoin: click-to-pay handler");
    else
...
{
    // clicking on heavycoin: URLs creates FileOpen events on the Mac:
